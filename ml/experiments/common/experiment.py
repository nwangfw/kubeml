...
class KubemlExperiment(Experiment):
    ...
    def save(self, path: str, function_name:str) -> None:
        """Saves the experiment with the id as name in pickle format and as a pandas daframe"""

        # convert it to dataframe
        d = self.to_dataframe()

        if not os.path.exists(f"{path}/{function_name}"):
            # if the demo_folder directory is not present 
            # then create it.
            os.makedirs(f"{path}/{function_name}")  
        _path = f'{path.rstrip("/")}/{function_name}/{self.network_id}.pkl'
